...
@section('main-content')
    <div class="card shadow">
        <div class="card-header shadow">
            <h5 class="text-primary text-uppercase font-weight-bold">Sửa danh mục sản phẩm</h5>
        </div>
            ...
            <form action="{{ route('category.update', $category->id) }}" method="post">
                ...
                <div class="form-group">
                    ...
                    <select id="status" name="status" class="form-control">
                        <option value="active" {{ $category->status == 'active' ? 'selected' : '' }}>Có hiệu lực</option>
                        <option value="inactive" {{ $category->status == 'inactive' ? 'selected' : '' }}>Vô hiệu</option>
                    </select>
